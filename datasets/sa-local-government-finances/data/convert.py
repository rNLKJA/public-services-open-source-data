...
(South Australia local general government, Table 334; total local general
government all-states, Table 339) into tidy long-format CSVs.
...
SA_FILE = RAW_DIR / "55120do015-south-australia-local-general-government.xlsx"
TOTAL_LOCAL_FILE = RAW_DIR / "55120do019-total-local-general-government.xlsx"
...
SA_SHEETS = [
    ("Table_1", "operating_statement", "sa-operating-statement.csv",
     "South Australia Local General Government Operating Statement"),
    ("Table_2", "cash_flow_statement", "sa-cash-flow-statement.csv",
     "South Australia Local General Government Cash Flow Statement"),
    ("Table_3", "balance_sheet", "sa-balance-sheet.csv",
     "South Australia Local General Government Balance Sheet"),
    ("Table_4", "expenses_by_purpose", "sa-expenses-by-purpose.csv",
     "South Australia Local General Government Expenses by Purpose (COFOG)"),
]

TOTAL_LOCAL_SHEETS = [
    ("Table_1", "operating_statement", "all-states-total-local-operating-statement.csv",
     "Total Local General Government Operating Statement (all states/territories combined)"),
]
...
def main():
    ...
    for sheet_name, table_key, out_file, title in SA_SHEETS:
        ...
        table_index.append({
            "table": table_key,
            "title": title,
            "geography": "South Australia (local government sector total)",
            "rows": len(records),
            "file": out_file,
        })
    ...
    total_wb = openpyxl.load_workbook(TOTAL_LOCAL_FILE, data_only=True)
    for sheet_name, table_key, out_file, title in TOTAL_LOCAL_SHEETS:
        records = parse_sheet(total_wb[sheet_name])
        write_csv(records, OUT_DIR / out_file)
        table_index.append({
            "table": table_key,
            "title": title,
            "geography": "Australia (all states/territories local government sector total)",
            "rows": len(records),
            "file": out_file,
        })
        print(f"{out_file}: {len(records)} rows")
    ...
    assert lookup(op_records, "Total GFS revenue", "2024-25") == 3599
    assert lookup(op_records, "GFS Net operating balance", "2024-25") == 568
    exp_records = parse_sheet(sa_wb["Table_4"])
    assert lookup(exp_records, "Total Expenses", "2024-25") == 3031
    assert lookup(exp_records, "Recreation, culture and religion", "2024-25") == 770
    bs_records = parse_sheet(sa_wb["Table_3"])
    assert lookup(bs_records, "GFS NET WORTH", "2024-25") == 36551
